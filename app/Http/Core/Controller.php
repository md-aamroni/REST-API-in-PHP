...
class Controller
{
	 ...
	 * @return string|bool
	 */
	public function encode($str, $filter = null)
	{
		switch ($filter) {
			case 'email':
				$sanitize = filter_var(trim(addslashes($str)), FILTER_VALIDATE_EMAIL);
				break;
			case 'int':
				$sanitize = filter_var(trim(addslashes($str)), FILTER_SANITIZE_NUMBER_INT);
				break;
			case 'url':
				$sanitize = filter_var(trim(addslashes($str)), FILTER_SANITIZE_URL);
				break;
			case 'ip':
				$sanitize = filter_var(trim(addslashes($str)), FILTER_VALIDATE_IP);
				break;
			case 'html':
				$sanitize = filter_var(trim(addslashes($str)), FILTER_SANITIZE_SPECIAL_CHARS);
				break;
			case 'bool':
				$sanitize = filter_var(trim(addslashes($str)), FILTER_VALIDATE_BOOL);
				break;
			default:
				$sanitize = filter_var(trim(addslashes($str)), FILTER_SANITIZE_STRING);
				break;
		}
